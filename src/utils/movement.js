...
export default class MovementUtils {
   ...
   *                     which has entity.directionVector,
  ...
  static fourWayMovement(entity, keys) {
    return onUpdate(() => {
      ...
      entity.directionVector = vec2(vel.x, vel.y);
      if (!entity.directionVector.isZero()) {
        const velocity = entity.directionVector.unit().scale(entity.speed); // unit() to fix diagonal movement
